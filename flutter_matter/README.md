...
In Section [Supported Clusters](#supported-clusters) the currently implemented clusters with their attributes and commands are listed.

## Quickstart 

1. Add a `dependency` on `flutter_matter: ^0.0.1` to your pubspec.yaml
...
// Read on/off attribute
 try {
    final isOn = await flutterMatterPlugin.onOffCluster
        .readOnOff(deviceId: 123, endpointId: 1);
...
try {
    await flutterMatterPlugin.onOffCluster
        .toggle(deviceId: 123, endpointId: 1);
...
// Subscribe to the on/off attribute value
final streamSubscription = flutterMatterPlugin.onOffCluster
    .subscribeOnOff(deviceId: 123, endpointId: 1)
    .listen(
      (isOn) => print('Light is ${isOn ? 'on' : 'off'}'),
...
1. Add `Matter Extension` to your Project
    ...
    2. In Xcode, select File -> New -> Target from the menu bar. <br/> ![File -> New -> Target](../docs/assets/setup_ios_1_2.png)

    3. Select `Matter Extension`, click next. <br/> ![Matter Extension](../docs/assets/setup_ios_1_3.png)
    
    4. Enter a `Product Name`, select your `Team` and make sure the selected `Project` and `Embed in Application` is the Flutter Runner Application <br/> ![Settings for the Extension](../docs/assets/setup_ios_1_4.png)
...
2. Add to your *apps's* **and** *extension's* `Info.plist` or add them by choosing the targets, click on info and add them in the `Custom iOS Target Properties`:
    ...
    ```
    ![Custom properties](../docs/assets/setup_ios_2_1.png)
...
3. Add App Groups to your *app* **and** *extension*:
    ...
    ![Add capabilities](../docs/assets/setup_ios_3_1.png)

    2. Search for `App Groups` and double click it
    ![Add App Groups](../docs/assets/setup_ios_3_2.png)

    3. On the Runner's target add new App Groups. On the extension's target just select it
    ![Create new App Groups](../docs/assets/setup_ios_3_3_1.png)
    ![Name the new App Groups](../docs/assets/setup_ios_3_3_2.png)
...
5. Add files to your extension
    ...
    The easist way to do this is to right click the extension's folder -> Add files to "Runner"... -> Select the  [Framework Helpers](/flutter_matter_ios/ios/Classes/Framework%20Helpers/) folder -> Make shure to select `Copy items if needed`, `Create groups` and your extension is selected as target -> Click Add
     ![Add files to extension](../docs/assets/setup_ios_4_1.png)
...
## Descriptor
This cluster describes an endpoint instance on the node, independently from other endpoints, but also allows composition of endpoints to conform to complex device type pattern.
...
| :-: | :-------------------- | :--------------------------- |
| -1  | General Error         | GeneralException             |
| -2  | Unsupported Platfrom  | UnsupportedPlatformException |
| -3  | User cancelled        | UserCancelledException       |
