...
class OnOffCluster: FlutterMatterHostOnOffClusterApi {
    ...
    private func getCluster(deviceId: Int64, endpointId: Int64) throws -> MTRBaseClusterOnOff {
        ...
        guard let cluster = MTRBaseClusterOnOff(device: device, endpointID: NSNumber(value: endpointId), queue: DispatchQueue.main) else {
            throw FlutterError(code: "-1", message: "Missing cluster", details: nil)
        }
        return cluster
    }
    ...
    func off(deviceId: Int64, endpointId: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        Task {
            do {
                ...
                completion(.failure(FlutterError(code: "-1", message: "Failed to send command off", details: nil)))
            }
        }
    }
    ...
    func on(deviceId: Int64, endpointId: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        Task {
            do {
                ...
                completion(.failure(FlutterError(code: "-1", message: "Failed to send command on", details: nil)))
            }
        }
    }
    ...
    func toggle(deviceId: Int64, endpointId: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        Task {
            do {
                ...
                completion(.failure(FlutterError(code: "-1", message: "Failed to send command toggle", details: nil)))
            }
        }
    }
    ...
    func readOnOff(deviceId: Int64, endpointId: Int64, completion: @escaping (Result<Bool, Error>) -> Void) {
        Task {
            do {
                ...
                completion(.success(result == 1 ? true : false))
            } catch {
                os_log(.error, "Failed to read attribute OnOff: \(error)")
                completion(.failure(FlutterError(code: "-1", message: "Failed to read attribute OnOff", details: nil)))
            }
        }
    }
    ...
    func subscribeToOnOff(deviceId: Int64, endpointId: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        Task {
            do {
                let cluster = try getCluster(deviceId: deviceId, endpointId: endpointId)
                let params: MTRSubscribeParams = .init(minInterval: 1, maxInterval: 10)
    ...
    func unsubscribeToOnOff(deviceId _: Int64, endpointId _: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        // TODO: How? ¯\_(ツ)_/¯
        completion(Result.success(()))
    }
}
